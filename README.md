...
| **implement-this** | Applies the implementation prefix as a fixed template — invoke as `/implement-this #<n>` to run `/implement` → `/code-review` with `/unslop` active. User-invoked only; issue reference replaces `Issue #0` verbatim. |
...
| You have a ticket number and want the implementation workflow without pasting the prefix | implement-this | /implement-this #<n> (user-invoked) |

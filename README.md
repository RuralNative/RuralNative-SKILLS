...
**Cleanup.** A successful worktree closes through supported host operations only after exact source recovery is proven: terminal success, a clean worktree, and one matching local/remote/PR head SHA, plus durable pull-request evidence. Workers never stop themselves; only the command session may request cleanup, and stop and removal are separate decisions. On today's Kilo chat side, stopping a session does not remove its checkout, so a stopped, remotely recoverable worker reports `cleanup-pending` and leaves everything in place rather than touching directories behind Agent Manager. Failed worktrees stay on disk for diagnosis, and `needs-info`, dirty, unpushed, and interrupted workers stay live with their worktrees (`preserved-for-resume` or `preserved-for-diagnosis`). A session missing while its worktree remains reports `recovery-required` instead of being duplicated or deleted. Cleaning up worktrees that existed before this workflow is a separate audited task outside the workflow itself.

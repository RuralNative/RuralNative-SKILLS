...
| Skill | What it does |
|---|---|
| **document-for-agents** | Keeps a project's agent-facing documentation honest — the doc-cache lifecycle: establish, audit, maintain. Requires `unslopify` before any user-visible prose and again before publishing. |
| **document-for-humans** | Derives plain-language docs (overview, decision journal, guardrails, data-flow story) for the people who don't read code — regenerated by agents, never trusted by them. Requires `unslopify` before any user-visible prose and again before publishing. |
...
| Agent docs need establishing, auditing, or maintenance | document-for-agents | Establish / Audit / Maintain — loads `unslopify` before prose, final audit before publishing |
| Human docs need establishing, auditing, or regeneration | document-for-humans | Establish / Audit / Maintain — loads `unslopify` before prose, final audit before publishing |
...
document-for-agents' own first move after loading. `unslopify` is both
standalone and a hard dependency: run it directly on prose you name when you
need standalone cleanup, or let `document-for-agents` and `document-for-humans`
load it before any user-visible prose and run a final audit before publishing.
Standalone scope is caller-provided; under a parent skill the parent's chosen
scope governs — routine maintenance passes changed prose, an audit may request a
repository sweep. Parent decisions, tier routing, glossary terms, invariants,
and approval gates outrank a prose rewrite; see the hard-dependency sections in
both parent skills. Missing `unslopify` stops the parent workflow with `npx
skills add RuralNative/RuralNative-SKILLS --skill unslopify`; missing Python
does not stop it.
...
1. **Install a skill** (recommended):
     ...
     if it is absent (missing Python does not stop them):
     ...
     an audit may request a repository sweep — parent scope governs.
...
- **For the skills: just a codebase** you want an AI agent to work in. Both
  ...
  `skills/unslopify/scanner.py` scanner does not stop it. Otherwise no runtime
  or framework is required. `document-for-humans` additionally expects an
  agent-first doc tree (the other skill's output) to derive from.

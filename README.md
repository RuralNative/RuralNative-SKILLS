...
- **Mistake records stay yours.** An optional private log of confirmed agent mistakes exists only with your consent to create and maintain it, announces every write in advance, stops on revocation before a separate keep, export, or delete choice, stays outside the doc cache, version control, and every agent read set, holds sanitized summaries with no prompts, code, secrets, personal data, absolute paths, or repository remotes, and never uploads — submission to the skill developer is manual after your review (ADR-0018).

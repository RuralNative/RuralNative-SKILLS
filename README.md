...
| **unslopify** | Removes AI tells from explicit prose while keeping meaning — the 31-pattern cleanup you run on named files or changed prose. |
...
| Prose reads as model-generated or needs plain-language cleanup | unslopify | Scan and rewrite explicit scope |
| Documentation cleanup before publishing | unslopify | Final audit on named files |
...
document-for-agents' own first move after loading. `unslopify` is
standalone: run it directly on prose you name, or a parent workflow may
provide its scope. Its hard dependency from the documentation skills is
approved in ADR-0005 and will be wired in a later slice.
...
1. **Install a skill** (recommended):
    ...
    Install the two doc skills if you want the full system: the agent doc tree
    first, the human view derived on top of it. Install `unslopify` alone when
    you need prose cleanup without the doc-cache workflow.

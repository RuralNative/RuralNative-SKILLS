...
a specific job. Most skill names start with a verb and name their audience:
you read the name and know what it does and who it's for. One utility,
`unslopify`, is audience-neutral because its cleanup applies unchanged to any
prose (ADR-0005).
...
| **implement-this** | Applies the implementation workflow to one issue — invoke as `/implement-this #<n>` directly for direct-main delivery or let an Agent Orchestrator worker use pull-request delivery through `/implement` → `/code-review` with `/unslop` active. |
| **supervise-this** | Coordinates a durable Agent Orchestrator run via `/supervise-this <task>` and `/supervise-this #<spec>` — plans inline, starts up to three Kilo Code workers behind GitHub native blockers, stays active after worker creation, follows pull requests and AO completion events, and continues through merge, recovery, and whole-spec review. |
...
| Prose reads as model-generated or needs plain-language cleanup | unslopify | Scan and rewrite explicit scope (standalone) |
...
| You have a ticket number and want the implementation workflow without pasting the prefix | implement-this | /implement-this #<n> direct, or one issue through an AO worker |
| You want a full durable run from planning through implementation | supervise-this | Run /supervise-this <task> inside an AO project orchestrator with configured role profiles |
| You interrupted a supervised run and want to resume | supervise-this | /supervise-this #<spec> resumes from GitHub and AO state |
...
not separate skills; see AI-First Workflow Integration for what each mode does.
`unslopify` works both standalone and as a hard dependency. Standalone scope is
caller-provided, parent scope governs; see Getting Started for order and
see Technical Requirements for the Python note. Parent decisions, tier routing,
glossary terms, invariants, and approval gates outrank a prose rewrite. Missing
`unslopify` stops the parent workflow with `npx skills add
...
- **The agent loads them on its own.** Once installed, the agent picks the
  ...
  manual setup each time. See Which skill, when for which skill and mode.
...
See Getting Started for how to install and see Technical Requirements for what each skill needs.
...
1. **Install a skill** (recommended):
     ...
     Install the shared dependency first — both documentation skills require it
     before any user-visible prose, and they stop with an install instruction
     if it is absent (missing Python does not stop them). Installed runtime
     ...
     then the agent doc tree, then the human view derived on top of it. Install
     `unslopify` alone when you need standalone prose cleanup without the
     doc-cache workflow. Routine doc work passes changed prose to `unslopify`,
     an audit may request a repository sweep; parent scope governs. See AI-First
     Workflow Integration for the three modes and Which skill, when for which
     situation triggers which mode.

2. **No npx? Copy manually** — clone the repo, then copy the skill folder(s)
     into your agent's skills folder. Keep the folder names. Install
     `unslopify` before the two documentation skills:
...
- **For the skills: just a codebase** you want an AI agent to work in. Both
  `document-for-agents` and `document-for-humans` require `unslopify` as a hard
  dependency — install it before running a documentation workflow. The workflow
  stops with `npx skills add RuralNative/RuralNative-SKILLS --skill unslopify`
  if the dependency is absent; missing Python for the optional
  `skills/unslopify/scanner.py` scanner does not stop it. Installed runtime
  resolves `unslopify` by skill identity, not by a repository-relative path.
  Otherwise no runtime or framework is required. `document-for-humans`
  additionally expects an agent-first doc tree (the other skill's output) to
  derive from.

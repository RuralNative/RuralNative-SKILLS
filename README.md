...
| **supervise-this** | Coordinates model-aware planning through happy-path execution via `/supervise-this <task>` and `/supervise-this #<spec>` with explicit planning, implementation, and optional review model and variant resolved through `agent_manager_models`, approval before execution, a local `plan-this` delegation, bounded worktrees that delegate to `implement-this` with the implementation selection, and integrated `code-review` with the review selection. |

...
`plan-this` is invoked directly by the user as `/plan-this <task>`. It applies the planning workflow and inserts the invocation task under `## Task:`, then defines an intent capsule, asks only the choices a human must decide, optionally explores directions, resolves the decision frontier, and delegates to `/grill-with-docs` → `/to-spec` → `/to-tickets` with `/unslopify` active. One direct invocation authorizes that full interactive chain, but nothing publishes until the run has recorded a complete, settled intent capsule, resolved its frontier, shown the plain-language preview with the specification outline and proposed ticket graph, and received your explicit approval; an interrupted run resumes its recorded decision tree. A complete task with a settled solution reaches the preview without a forced question. Unrelated invocation is rejected and no second planning contract is created.

Before publication, planning records an `ordinary` or `high-risk` class for every ticket. Evidenced security boundaries, migrations, shared contracts, broad public interfaces, dependency changes, and comparable blast radius are high-risk. Later implementation or review may raise it with evidence but may not lower it.
...
Every published criterion carries a local ID unique within its own issue, written `- [ ] AC-1: text`. New records start unchecked; a checked box is never completion evidence and never retires a criterion. The stable criterion key is the authority issue number plus that local ID, so two issues may both use `AC-1`. Clearer wording keeps the same ID; changed observable behavior gets a new ID and retires the old one with an explicit `(retired)` marker, and retired IDs are never reused or renumbered.

Before publication approval, planning resolves an orientation set for every proposed ticket from its affected seams (ADR-0024, ADR-0032): the compact architecture index, whole affected seam leaf docs, leaf-named glossary entries, and linked accepted ADRs or policies, recorded before broad loading. Length alone never rejects a ticket; a cache-gap approval may substitute or narrow the set. The approval preview stays in plain language; the published ticket stays focused on behavior and sufficient verification. Affected seam names remain the durable join key; no ticket field transports paths, section anchors, invariant lists, glossary excerpts, or policies.
...
A healthy run loads `/unslopify` before the first progress update and keeps it active throughout the workflow, defines an intent capsule (`Outcome`, `User`, `Why now`, `Success`, `Constraints`, `Non-goals`) without asking when repository facts and the confirmed task decide every field, optionally explores directions when the solution form is unsettled, asks only the remaining choices a human must decide, one plain question at a time with the options the decision requires and one clear recommendation, shows the preview that explains what will be created, the main risks, and what happens next together with every split boundary, waits for your explicit approval before calling `/to-spec` and `/to-tickets`, runs those three delegated skills in order, and places the full task `Create a Next.js App` verbatim under `## Task:` without truncation or other substitution — matching the structured `SKILL.md` workflow (workflow line, authorization paragraph, `## Contract` plus five numbered phases, final `**Why / What / Where / How**` line, and single `## Task:` slot). Completion stops after its ELI18 summary.

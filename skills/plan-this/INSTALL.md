...
A healthy run loads `/unslopify` before the first progress update and keeps it active throughout the workflow, defines an intent capsule (`Outcome`, `User`, `Why now`, `Success`, `Constraints`, `Non-goals`) without asking when repository facts and the confirmed task decide every field, optionally explores three directions when the solution form is unsettled, asks only the remaining choices a human must decide, one plain question at a time with at most three short options and one short recommendation, shows the preview that explains what will be created, the main risks, and what happens next together with every split boundary, waits for your explicit approval before calling `/to-spec` and `/to-tickets`, runs those three delegated skills in order, and places the full task `Create a Next.js App` verbatim under `## Task:` without truncation or other substitution — matching the structured `SKILL.md` workflow (workflow line, authorization paragraph, `## Contract` plus five numbered phases, final `**Why / What / Where / How**` line, and single `## Task:` slot). Completion stops after its ELI18 summary.

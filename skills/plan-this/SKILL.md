...
- Record `ordinary` or `high-risk` on every ticket before publication. Security boundaries, migrations, shared contracts, broad public interfaces, dependency changes, and comparable evidenced blast radius are high-risk. Plan for no more than three workers per stage and four active managed workers across the workspace.

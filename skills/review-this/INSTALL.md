...
- Node 24 or newer: the bounded `prepare-review.mjs` entry point, the review-only `publish-review.mjs` entry point, and the read-only `workflow-cli.mjs` checks ship in this package and fail closed on older runtimes.

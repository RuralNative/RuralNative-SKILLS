...
4. Validate the current checkout with `checkoutMatchDecision` from `review-session.ts`: local `HEAD` must equal the selected pull-request head SHA, and the worktree must be clean. Branch names are informational only: an alias, `main`, or detached `HEAD` at the same commit matches. A dirty worktree or a different commit stops instead of checking out or creating another worktree.

...
Always run the local Standards and Spec review. Follow `REVIEW.md` as the cross-cutting policy. The initial revision receives one full review. Spawn the Standards and Spec sub-agents in parallel in fresh contexts so their reasoning stays separate, then aggregate both reports side by side under `## Standards` and `## Spec`, verbatim or lightly cleaned. Inspect impacted callers and surrounding seams when evidence requires it, but report only defects introduced, exposed, or made relevant by the pull request. Every Standards result records security, performance, correctness and edge cases, style, tests and test bloat, and documentation as `passed`, `not applicable`, `advisory`, or `blocking`. Never merge or rerank findings across axes. Hosts that cannot provide the persistent worktree and fresh nested agents stop before a verdict. Use focused doc-cache loading and pin each finding to the exact head and base SHAs it reviewed.

...
- Never apply fixes, edit pull-request source, commit, push, merge, update the pull-request body, update labels, promote dependents, or close tickets. Publication of the review and inline findings is terminal. The only working-tree effect in this run is checkout alignment to the verified pull-request head commit in Prepare; the reviewer authors no repository file.
...
Publish the pinned review with `ReviewPublishAdapter` from `adapters.ts`: one review body with `## Standards` and `## Spec`, verified inline findings quoting the offending span at its pinned file and line, the pinned head, base, requirements revision, and review-policy revision, the verification evidence, and exactly one `review-handoff-v1` block rendered by `renderReviewHandoff` from `workflow-state.ts` for the `fix-this` consumer. Read back the published body and validate it with `validateReviewHandoff` before finishing; an unreadable or incomplete publication stops with a diagnostic instead of claiming a usable handoff. Then stop, and direct the user to run `/fix-this <PR-number>` for the final stage; never launch it from this run.

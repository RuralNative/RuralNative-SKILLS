...
Confirmed findings return to the ticket's owning worker rather than being fixed in the review workspace in the currently shipped path. For the persistent PR path, findings are published only after the trusted summary is upserted and inline findings are verified; a publication failure stops later fixes and merge. The persistent PR worker coordinates one fresh fix subagent inside the same PR worktree per round; no separate review or fix worktree is created. At most two code-fix rounds are allowed. A conflict-free base refresh and infrastructure retry consume no round; conflict resolution consumes one. Advisory-only findings never create a fix round alone.

## Merge

A pull request is eligible only when `isMergeEligible` from `workflow-state.ts` passes. Merge requires green required checks, resolved confirmed findings, a clean local review, and an unchanged reviewed head SHA and base SHA pair, plus a published trusted summary and a mergeable pull request. The reviewer verifies every finding against the current revision before publishing it; sub-agent output never merges, approves, closes, or labels anything. Eligible pull requests squash-merge and their closing references close the assigned tickets. Merge uses squash-merge with the pull-request body `Closes #<ticket>` so GitHub closure follows delivery evidence. Never close a ticket before merge and never force-push.

...
The lifecycle managed by `supervise-this` inside an AO project orchestrator. AO keeps the orchestrator persistent after planning and worker creation. The supervisor runs `/plan-this` inline, records the AO project and role profiles on the parent issue, reads GitHub's child issues and native blockers before AO sessions, and starts at most three Kilo Code TUI workers with documented `ao spawn` calls. The AO worker profile supplies implementation models and the orchestrator profile supplies planning and final review models; the supervisor does not invent per-spawn model flags or rewrite profiles during a run. Each worker runs `/implement-this #<n>` in pull-request delivery, so the worker does not push directly to `main` or close the issue before merge. AO owns worktrees, CI and review feedback, merge conflicts, and session recovery; the supervisor owns the GitHub dependency frontier, acceptance evidence, labels, issue closure, follow-up tickets, and parent closure. AO completion or handoff events cause reconciliation rather than a terminal summary. Resume reads GitHub before AO sessions and does not duplicate a specification, ticket, worker, or pull request. One mechanical recovery message is allowed; a still-blocked ticket receives `needs-info` and one ELI18 decision while unrelated ready work continues. After planned pull requests merge, the orchestrator runs full verification and `/code-review` from a fixed base, permits two automatic follow-up and review rounds, then closes the parent only after merged pull requests, evidence, checks, and final review pass.

...
**Fix progress**:
The versioned `fix-progress-v2` checkpoint `fix-this` maintains for resumable finalization: target, ticket, and parent, the source review, a digest over the deterministic rendering of the validated source handoff, started and resulting revisions, complete finding dispositions, verification receipts, the intended remote operation, completed steps, and the confirmed merge receipt. It reconciles reruns against observed facts, never authorizes work alone, and only a trusted checkpoint lets a confirmed merged PR resume bookkeeping. A legacy `fix-progress-v1` checkpoint is diagnostic input, never permission to skip work (ADR-0035, ADR-0038).
...
**Evidence repair**:
The workflow-owned `evidence-repair-v1` record review preparation writes when it repins scoped evidence: old and new requirements revisions, old/new head and base, reason, and verification provenance. The observation-based `recover-evidence` operation is the sole writer (`repair-record` only inspects and reconciles an existing attempt); it derives classification, scope, block validity, proof, and RED preservation from observation, establishes checks from the pinned configuration and approved frozen boundary, requires a complete recorded receipt for the observed verification intent and every active criterion including non-behavior proof, rejects truncated output and downgraded declared-behavior records, and places exactly one repair record inside the single replacement region. It re-reads the full PR body, pinned inputs, positive head-repository identity, and caller-observed governing sources before each write, replaces exactly one validated evidence region preserving surrounding bytes, then reads back and revalidates; identical records are reused after interruption, and races are reported (ADR-0039).

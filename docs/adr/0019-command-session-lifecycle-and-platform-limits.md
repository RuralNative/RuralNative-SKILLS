...
- Ticket #154 shipped the planning grill gate and parallel-first ticket graphs. Tickets #155 through #158 remain pending behavior work. Until each pending ticket merges, the affected seam leaves describe current shipped behavior; this ADR records the accepted target for the remaining tickets.

Activation: this decision governs spec #152 from today. Ticket #154 ships the planning grill gate and parallel-first ticket graphs in `plan-this`. Tickets #155 through #158 move `implement-this` and `review-this` onto the remaining behavior described here. Until each remaining behavior ticket merges, its seam leaf keeps describing shipped behavior and marks the pending amendment. Platform statements rest on the pinned Kilo Agent Manager sources in `reference/vendor-facts.md`, retrieved 2026-08-23.

...
- Ticket #154 shipped the planning grill gate and parallel-first ticket graphs. Ticket #155 shipped implementation command sessions, #156 shipped target resolution, and #157 shipped risk, timing, setup reconciliation, and persistent initial review contracts. Ticket #158 remains pending behavior work. Until it merges, its seam leaf describes current shipped behavior; this ADR records the accepted target for the remaining ticket.

Activation: this decision governs spec #152 from today. Ticket #154 ships the planning grill gate and parallel-first ticket graphs in `plan-this`; #155 and #156 ship implementation workers and target resolution; #157 ships the performance contract and persistent initial review path. Ticket #158 moves `review-this` onto the remaining fix and delta-review behavior. Until #158 merges, its seam leaf keeps describing shipped behavior and marks the pending amendment. Platform statements rest on the pinned Kilo Agent Manager sources in `reference/vendor-facts.md`, retrieved 2026-08-23.

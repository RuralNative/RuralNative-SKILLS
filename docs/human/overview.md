...
`plan-this` runs the planning interview, specification, and ticket workflow with repository-owned `unslopify` and the focused agent cache `AGENTS.md → ARCHITECTURE.md → seam leaf → CONTEXT.md → relevant ADRs`, whose loading budgets are caps on orientation documents; `AGENTS.md` opens with a five-command working contract, and a named cache gap requires owner approval before the read set widens. Human pages are derived from agent docs and not preloaded. `implement-this` validates one ticket, several ready tickets, or a parent specification against the frontier, then runs each ticket through an isolated worker and a single `/implement` stage under the same prose and cache rules; it does not host code review. `review-this <target>` owns one pull-request review wave for its parent specification: you can point it at the parent specification, a child issue, a pull request, or a full URL, and bare numbers (`100`) and hash numbers (`#100`) mean the same thing. It resolves that target to its exact GitHub objects and pull-request set before any worktree is created or anything is written — a wrong or ambiguous target names what stopped it — then discovers native child tickets, linked pull requests, blockers, checks, reviews, and current head and base SHAs, reconciles Kilo cloud review with the local Standards and Spec review against each current head, routes confirmed findings to the owning worker, squash-merges clean heads, promotes newly unblocked dependents, and closes the specification when final verification and whole-spec review pass. ADR-0019's accepted design starts implementation and review as independent command sessions, with each session managing its own frontier. Target resolution shipped with #156; tickets #154, #155, #157, and #158 implement the remaining behavior, so the current commands retain the behavior described in their seam leaves until those tickets merge. Every ticket delivers by pull request.
...
This example describes the accepted ADR-0019 design. Target resolution shipped with #156; tickets #154, #155, #157, and #158 have not shipped, so the current commands retain the behavior described in the seam leaves for those changes.

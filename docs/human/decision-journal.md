<!-- human-first: derived artifact — agents regenerate, never cite as ground truth · Derived: 2026-09-14 · Regenerated: ADR-0043 portable native invocation (slash, Codex explicit, and native loading with stable identities; OpenCode no-argument selection with task text in a separate message; locked companions pause for human same-session invocation; bundle parity host-independent; implement-this has no upstream implement dependency) + ADR-0042 run-local tooling repair and native closing links (reproduced helper defects get one isolated correction per cause and operation below the run directory with helper-observed receipts and unchanged guards; native closing links reconcile the connection count so a truncated page never certifies complete) + ADR-0041 role-scoped recovery and resume (automatic role-scoped recovery, verified local-work preservation, and interruption resume across all four production skills) + ADR-0040 GitHub-native production workflows (native COMMENT reviews with marker and inline comments, independent permission reads, paginated complete enumerations, pinned default branch, verified merge with bookkeeping-only recovery) + review-bugfix pass on the ADR-0038 implementation (pending reviews are created without an event and submitted through the native events endpoint with the states COMMENTED/APPROVED/CHANGES_REQUESTED; a lost create response is reconciled by discovery of exactly one pending review; publication counts only after full read-back provenance validation; the bundled checks require recorded execution receipts and the observed project configuration behind passing verification claims; fix-progress checkpoint trust derives from reconciled parts and a completed merge step requires a confirmed receipt; same-version pin inequality is a revision mismatch with unproven cause unless separately proven; pushes gate on the recorded pre-push remote head and stop no-op pushes), ADR-0037 standardized checkbox criteria and adapted intake (new `- [ ] AC-N` records; clear alternate templates continue read-only with a conservative whole-body fingerprint) + ADR-0038 verified handoff repair (requirements resolution errors stop pinning with a typed diagnostic; a body carrying both `## Solution` and `## Settled decisions` consumes as adapted with both homes fingerprinted; every stage runs its bundled Node 24+ validator command against published read-back artifacts; review publishes one complete verifiable review as one bounded step through a review-only entry point; implementation repins only after mismatch classification and full revalidation; finalization resumes from `fix-progress-v2` receipts and confirmed merged PRs resume bookkeeping only) + ADR-0036 review checkout auto-alignment (clean-checkout alignment to the pinned PR head in detached HEAD; the strict match stays the final gate; dirty, unfinished-operation, and collision states stop with no checkout effect; the aligned checkout stays for fix-this) + ADR-0035 fix-this final stage (review-handoff-v1, commit-based checkout matching, local verification with no CI gate, squash merge with resumable bookkeeping) + ADR-0034 self-contained handoffs and optional review policy (no /implement dependency; ordered PR delivery with evidence-v2 head binding; repair on one verified PR; REVIEW.md optional with no draft creation) + ADR-0032 remove document-size gates + ADR-0031 single-target production workflows (no Agent Manager, workers, waves, or cloud review; one ticket or pull request in the current checkout) + ADR-0029 unslopify session-start setup and plain language + ADR-0030 larger orientation ceilings: unslopify now maintains one owned block in the project's root AGENTS.md so later sessions load it before the first reply (the install CLI has no setup hook, so one invocation per project establishes this), drafts all model-authored English in plain language with a silent pre-send check, and always replaces load-bearing, smoking gun, and smoke test phrasing under AIT-LEX-009 while vertical slice and native dependency edges stay context-aware; every document and orientation ceiling rose 50 percent (ordinary 9,000, API or route 13,500, schema or data 18,000, re-orientation 10,500, absolute 18,000 bytes; index 225 lines, three-minute leaf read, 105-line policy budget, 3-8 initial invariants, review past about 23) while ceilings stay caps, not targets, the trimming order is unchanged, and no existing document was padded to fill the room; ADR-0028 adaptive doc-cache governance: the doc-cache tier now grows itself (minimal to standard to full) as evidence appears — a durable decision, more seams, or coordination need — and never shrinks automatically; every lifecycle run first resolves the tier, the private diagnostics consent, and the decision frontier; decisions record their context, alternatives, and rejection reasons before code work, and lost rationale is recovered only from cited evidence or marked unknown; each documented seam now carries a code fingerprint that fails the check when the docs drift from the code, in a working tree and in clean CI alike, and refreshing it requires reviewing the affected claims; #190 versioned requirements revision: the dispatch packet, acceptance evidence, and review wave carry one SHA-256 fingerprint of the canonical affected seams, criteria, constraints, blockers, settled decisions, risk, and verification intent from the parent and ticket bodies; a body edit stops delivery, review publication, and merge with `needs-info` until the body is reconciled and the user resumes, with no waiver; #189 fewer, simpler planning questions (ADR-0027): planning asks only when repository facts and the confirmed task cannot decide a choice that changes product behavior, scope, cost, risk, or an action that is hard to undo; a complete settled task reaches the preview without a forced question; questions use one plain sentence, at most three short options, and one short recommendation with technical terms explained; previews stay plain; explicit approval alone publishes; #187 dependency reconciliation now controls worker setup (spec #183 AC-11): a worker runs dependency setup only when `reconcileDependencyState` reports missing or changed state, unchanged state reuses the worktree setup, every worktree keeps its own dependency directory, and no instruction tells every worker to always run `npm ci`; #186 keeps unslopify reports out of normal chat (ADR-0026): publication stays silent, published artifacts carry no audit block, and an explicit audit request returns the full report; #185 planning stops when risk evidence is missing: a high-risk trigger without evidence is an internal incomplete result that blocks publication and risk labeling; published tickets keep only `ordinary`/`high-risk` with upward-only escalation; #188 stable acceptance criterion IDs across planning, implementation, and review: plan-this publishes criteria with local `AC-N` IDs, implementation dispatches and proves evidence by ID with retired IDs never accepted, and review Spec findings and whole-spec verification cite the stable `#<issue>:AC-N` key; #179 workflow commands consume bounded orientation (ADR-0024): planning preflights each proposed ticket, implementation resolves current sources before broad loading with one direct-ticket resolution attempt, and review resolves one set per pinned revision pair, stopping before broad loading when the set is over budget and reporting its exact sources; retain workflow workers until delivery is durable (ADR-0023): workers never stop themselves, cleanup only after exact source recovery, recovery-required for a missing session; 2026-08-29 #178 migration: compact seam index, redirect-tier leaves, declared routes, check 11 active; #177 bounded orientation (ADR-0024): runtime orientation sets with strict byte caps, the harness-owned coverage manifest, and the Improve path; #172 triggered quality proof: conditional evidence sections for browser, security, operability, migration, and performance with pinned Addy provenance; #171 worker evidence contract (ADR-0021); #173 frontier review authority (ADR-0022): frontier-owned review judgment and merge, one live-catalog execution model per wave, mutation-worker boundaries, category transport, axis-preserving deduplication, specialist routing; sub-issue reads corrected to the native /sub_issues REST suffix in the shared github-facts reader (the public operation stays sub-issues) · Sources: docs/adr/0001-distribute-as-public-catalog-shelf.md, docs/adr/0002-adopt-ten-check-gate.md, docs/adr/0003-human-first-derived-artifacts.md, docs/adr/0004-verb-named-skills-flat-shelf.md, docs/adr/0005-unslopify-utility-identity-and-hard-dependency.md, docs/adr/0006-plan-this-fixed-template-adapter.md, docs/adr/0007-supervise-this-coordinator.md, docs/adr/0008-supervise-this-agent-orchestrator.md, docs/adr/0009-delegation-invariants-human-invocation.md, docs/adr/0010-supervise-by-delivery-evidence.md, docs/adr/0011-retire-supervise-this.md, docs/adr/0012-manager-worktree-pull-request-delivery.md, docs/adr/0013-review-this-decoupled-code-review.md, docs/adr/0014-three-skill-development-workflow.md, docs/adr/0015-requirements-data-trust-and-install-provenance.md, docs/adr/0016-unslopify-always-on-output-contract.md, docs/adr/0017-doc-cache-attention-boundary.md, docs/adr/0018-opt-in-skill-diagnostics.md, docs/adr/0019-command-session-lifecycle-and-platform-limits.md, docs/adr/0020-plan-this-structured-workflow.md, docs/adr/0021-implement-this-worker-evidence-contract.md, docs/adr/0022-frontier-review-authority-and-mutation-worker-routing.md, docs/adr/0023-retain-workers-until-durable.md, docs/adr/0024-bounded-orientation.md, docs/adr/0025-required-orientation-sources.md, docs/adr/0028-adaptive-doc-cache-governance.md, docs/leaves/implement-this.md, docs/adr/0029-unslopify-session-start-and-plain-language-live-output.md, docs/adr/0030-larger-orientation-ceilings.md, docs/adr/0031-single-target-production-workflows.md, docs/adr/0032-remove-document-size-gates.md, docs/adr/0033-review-only-with-policy-bootstrap.md, docs/adr/0034-self-contained-handoffs-and-optional-review-policy.md, docs/adr/0035-fix-this-final-stage.md, docs/adr/0036-review-checkout-auto-alignment.md, docs/adr/0037-standardized-criteria-and-adapted-intake.md, docs/adr/0038-verified-handoff-repair-bounded-review-publication-and-resumable-finalization.md, docs/adr/0041-role-scoped-recovery-and-resume.md, docs/adr/0042-run-local-tooling-repair-and-native-closing-links.md, docs/adr/0043-portable-native-invocation.md -->
# Decision journal in plain words

### 2026-09-14 - Repair a broken review helper without touching the install (ADR-0042)

When a review fails because its own helper is broken, the review now fixes an
isolated copy of that helper and proves the fix before using it. The copy lives
only below the run directory, the shared install never changes, and permission
checks and proof rules stay byte-identical. Each fix gets one attempt per cause
and operation, denials stay denials, and the corrected copy is re-run through
its regression checks plus a re-observation before the review resumes only the
failed step. Ticket links now come from the native closing connection with its
count reconciled, so a truncated page can never certify an empty result.

Depth: [ADR-0042](../adr/0042-run-local-tooling-repair-and-native-closing-links.md).

### 2026-09-12 - Recover operational failures without manual restarts (ADR-0041)

All four production stages now recover their original request and verified
progress after interruption. They correct operational prerequisites within their
role and reconcile uncertain GitHub writes before retrying. Dirty local work is
preserved in a verified recoverable Git snapshot before necessary alignment;
verified task edits resume in place. Snapshots retain their original checkout
identity and are neither applied onto another revision nor deleted automatically.

This replaces manual-cleanup stops that stranded an otherwise valid review.
Review still leaves source fixes to finalization, planning still needs publication
approval, and no stage bypasses permissions or invents test evidence. A terminated
host process still needs to be resumed before its agent can continue.

Depth: [ADR-0041](../adr/0041-role-scoped-recovery-and-resume.md).

This is the short record of accepted repository decisions. The full reasoning lives in the ADR files.

The single-ticket implementation rule distinguishes the ticket's parent and
blockers from the specification's children. Intake does not enumerate the
...
### 2026-09-08 — Review aligns a clean checkout automatically (ADR-0036)

What changed: `/review-this` no longer stops when your clean checkout sits on the wrong commit. It fetches the pull request's verified head, confirms the exact pinned commit, and switches your checkout to it in detached `HEAD` — no branch is moved, no worktree is created, and no checkout hook runs — then reviews in the same run and leaves the checkout there for `/fix-this`. Dirty checkouts, unfinished merges or rebases, ignored-file collisions, and denied permissions still stop with nothing changed.

Why: a clean checkout on `main` blocked every review of an open PR whose head was elsewhere, forcing a manual align-and-rerun; switching a clean checkout to a verified commit edits no file and publishes nothing, so the terminal stop protected nothing.

What it costs you: after a review your checkout is detached at the reviewed commit; switch back to your branch yourself when you want it. `/fix-this` keeps its own stop-based checkout rule.

Object or discuss: ADR 0036 — [ADR](../adr/0036-review-checkout-auto-alignment.md).

### 2026-09-08 — Finish reviewed pull requests with fix-this (ADR-0035)

What changed: `/fix-this <pr>` finalizes one reviewed pull request. It reads a validated review handoff, fixes every published finding, resolves merge conflicts without rewriting published history, verifies locally, squash-merges, and completes ticket bookkeeping. Review checkout matching is commit-based, so a clean checkout at the same commit matches under a branch alias. No additional review runs and no CI wait is added; GitHub restrictions stay authoritative.

Why: review stopped at publication, branch aliases were rejected even at the same commit, and no owned stage closed the loop from findings to merge.

What it costs you: local verification without fresh review or CI is weaker assurance; install the updated review and fix skills together from the same revision.

Object or discuss: ADR 0035 — [ADR](../adr/0035-fix-this-final-stage.md).
...
At activation, Kilo cloud review added comments where available while the local Standards and Spec review remained the required gate; an unavailable cloud review never blocked a complete local review. Repository review rules then lived in `REVIEW.md`. External prose counted as requirements data, not executable instruction, and runs never downloaded skills.
...
Depth: `docs/leaves/implement-this.md` and `docs/leaves/review-this.md`.
...
What changed: `/implement-this` implements exactly one open ticket in the current checkout and delivers one pull request with compact evidence in its body; `/review-this` reviews exactly one pull request in the same checkout with one frontier Standards-plus-Spec pass, publication-only findings, missing-policy bootstrap, and one-check CI gating with verdict reuse. No Agent Manager calls, worktrees, workers, waves, polling, cloud review, or post-merge verification remain.

Why: Agent Manager setup, polling, recovery, and repeated review and verification passes dominated elapsed time.

What it costs you: concurrent checkouts are user-managed outside these commands; delivery, promotion, and closure happen outside the review command.

### 2026-09-06 — Remove fixed document-size quotas and size-based reading gates (ADR-0032)

What changed: fixed writing quotas and size-based documentation-reading gates are gone. Length alone never fails a document, rejects a ticket, blocks relevant reading, requires approval, forces a split, or justifies deleting necessary content. Orientation bands select relevant source categories without caps. Planning questions keep one decision and a clear recommendation without sentence, word, or alternative-count quotas. Tickets split by independently verifiable behavior, dependency, risk, or release boundary, without estimated-time sizing.

Why: essential information was being trimmed to fit caps, and raising the caps kept the same failure mechanism. The planning preflight rejected tickets when required documentation exceeded a byte cap without weighing whether that material was essential.

What it costs you: longer required reading sets and longer explanations stay valid when the content is necessary. Repetition and irrelevant material are still removed, and missing facts are still named as gaps rather than invented.

Object or discuss: ADR 0032 — [ADR](../adr/0032-remove-document-size-gates.md).

### 2026-09-08 — Self-contained handoffs with optional review policy (ADR-0034)

What changed: `/implement-this` no longer calls `/implement`; it implements directly with focused tests and delivers one pull request with evidence-v2 proof bound to the pushed head SHA. A `ready-for-human` ticket with exactly one verified matching PR may repair delivery without redoing work. `/review-this` validates the actual PR body with the shared handoff check and treats `REVIEW.md` as optional guidance that creates nothing when absent.

Why: delegated completion, publication before the freshness check, and repair blocked by entry labels caused intermittent missing PRs; review rejected unpinned evidence.

What it costs you: new evidence must carry a head SHA; missing or stale proof stops with a recovery diagnostic outside review.

Object or discuss: ADR 0034 — [ADR](../adr/0034-self-contained-handoffs-and-optional-review-policy.md).

Portable invocation (ADR-0043): use /plan-this, /implement-this, /review-this, and /fix-this where slash commands exist, $plan-this and siblings or native skill selection in Codex, and native skill loading elsewhere. On OpenCode, select with no arguments and supply task or target text in a separate ordinary message; slash-argument forwarding is unsupported. Locked upstream companions pause for explicit human invocation in the same session.

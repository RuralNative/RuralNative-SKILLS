...
### 2026-08-23 - Run the workflow through command sessions (decided)

The three workflow commands stay, and you start implementation and review yourself as separate command sessions whenever each stage has work. No per-wave handoff remains and nothing supervises them. Implementation makes pull requests; review reviews, fixes, and merges them; neither silently does the other's missing work. Confirmed fixes land in their own worktree per pull request instead of going back to the original implementation worker. Command sessions poll with growing pauses, pause after 30 idle minutes, and resume from GitHub without duplicates. Worktrees close only through supported Kilo actions; when chat cannot close one, the run reports `cleanup-pending` and leaves it alone. Failed worktrees stay on disk for diagnosis.
...
What it costs: the behavior updates roll out through tickets #154 to #158; until each merges, the skill pages describe shipped behavior.

...
- Planning publishes an unlabelled parent specification and child tickets linked as native GitHub sub-issues, builds native blocked-by edges from REST database IDs, and asks about unresolved decisions. `ready-for-agent` is the only claimable label; `ready-for-dev` is retired.

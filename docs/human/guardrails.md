...
- Implementation validates one ticket, several ready tickets, or a parent specification against the frontier before any claim, allows one parentless standalone ticket but rejects parentless multi-ticket sets, dispatches each ticket — including a single-ticket run — through `agent_manager` in worktree mode with one task and one initial prompt per ticket, reads status from the Agent Manager overview, uses the supported stop control, keeps at most three implementation workers and four managed workers across the workspace, loads repository-owned `unslopify` before progress with the same prose and focused-cache rules, stops when a native blocker is open, and stops before any claim if the host cannot isolate the work.

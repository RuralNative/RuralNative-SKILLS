...
11. **INV-11** — Retired by ADR-0031: state and adapter boundaries callable by a future persistent coordinator. Current: pure helpers for single-target resolution, finding validation, review-policy bootstrap, verdict reuse, CI equivalence, and review publication.
...
14. **INV-14** — Retired by ADR-0033: frontier owns verdict, commit, push, merge, and bookkeeping with an optional `review-fixer`. Current: the frontier reviewer publishes findings and stops; no fix subagent runs and no source edit, commit, push, merge, label, promotion, or closure occurs.
...
16. **INV-16** — Requirements revision plus review-policy revision: the published review pins head, base, requirements, and policy; reuse only when every key is unchanged; mismatch stops with `needs-info`; no waiver.
17. **INV-17** — CI publication: local review starts without waiting for CI; the publication gate reads required checks once and never polls; pending CI publishes the review and stops; absent equivalent CI runs the full local gate once as fallback.
18. **INV-18** — Missing review policy: after a valid target and a clean matching checkout, a missing root `REVIEW.md` creates at most that uncommitted draft from observed repository evidence and stops before publication; an existing policy is used unchanged; unreadable files, symlinks, conflicting sources, and creation failures stop with a diagnostic and create nothing.

## Verification

Equivalent required CI on the reviewed head/base, or the full local gate once as fallback; publication only, never delivery.

## Links

Glossary: `CONTEXT.md`. Decisions: ADR-0006, ADR-0014, ADR-0015, ADR-0019, ADR-0022, ADR-0023, ADR-0024, ADR-0031, ADR-0032, ADR-0033. Review policy: `REVIEW.md`. Redirect: `docs/leaves/ext/review-this.md`.

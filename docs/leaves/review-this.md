...
Owns one pull-request review wave: resolve targets, reconcile review, route findings, squash-merge, promote, close the spec.
...
5. **INV-5** — Target resolution/discovery: bare/hash identical; `selectReviewWave` returns open child PRs with valid closing refs, head/base SHAs.6. **INV-6** — Cloud absence/failure/timeout records `unavailable`, never blocks local review.

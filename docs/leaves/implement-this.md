...
`implement-this` applies the one-issue implementation workflow. It runs `/implement` and `/code-review`, keeps ticket authority and verification in one place, and chooses direct-main delivery for standalone use or manager-worktree pull-request delivery when the session runs inside a Kilo Agent Manager worktree.

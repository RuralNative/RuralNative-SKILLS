...
3. **INV-3** — The workflow contains the `/implement` → `/code-review` order, one `Issue #0` substitution point, ticket authority, blocker checks, and the repository verification command; the chain cannot be traversed unattended — `/implement` carries `disable-model-invocation: true` and requires explicit human invocation, while `/code-review` carries no such lock and remains model-invocable. Mechanism: composition test in `skills/implement-this/tests/` verifies the workflow order, flags `/implement` as human-invoked with `disable-model-invocation: true`, and flags `/code-review` as model-invocable with no such lock.

...
Doc-cache lifecycle: establish, audit, improve, maintain. Two outputs: cache accuracy and attention control.
...
8. **INV-8** — Loading rows and budgets are hard caps; a missing fact is a cache gap; widening needs owner approval.
9. **INV-9** — Leaf shape closes with a `Not here` route by stable responsibility, never a file path.
10. **INV-10** — Work conflicting with a numbered invariant stops until an approved decision supersedes or narrows it.

...
Planning workflow adapter, `/plan-this <task>`: confirms intent, explores when unsettled, resolves the decision frontier, publishes an approved parent spec with coherent child tickets. Focused route: `AGENTS.md → ARCHITECTURE.md → seam leaf → CONTEXT.md → ADRs`. Published criteria carry local `AC-N` IDs.
**Not here**: implementation → `implement-this`; review → `review-this`; prose → `/unslopify`.

## Non-negotiables

1. **INV-1** — `name` equals folder `plan-this`.
2. **INV-2** — INSTALL: `npx skills add ... --skill plan-this`, manual copy.
...
5. **INV-5** — Intent and decision gates: six-field capsule per run; one-at-a-time ELI18 decisions; three directions when unsettled.
6. **INV-6** — One planning contract: structured body is the single behavior source.
